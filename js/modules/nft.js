...
// ─────────────────────────────────────────────────────────────────────────────
// NFTDrive ローディングオーバーレイ
// ─────────────────────────────────────────────────────────────────────────────

(function setupNftDriveLoadingOverlay() {
    // スタイル追加（初回のみ）
    ...
    style.textContent = `
        ...
        #_nftdrive_overlay.active { display: flex; }
        @keyframes _nftdrive_spin { to { transform: rotate(360deg); } }
        #_nftdrive_overlay .spinner {
            width: 52px; height: 52px;
            border: 5px solid rgba(255,255,255,0.25);
            border-top-color: #fff;
            border-radius: 50%;
            animation: _nftdrive_spin 0.75s linear infinite;
        }
        #_nftdrive_overlay .label {
            color: #fff; font-size: 14px; letter-spacing: .5px;
        }
        #_nftdrive_overlay .hint {
    ...
    overlay.id = '_nftdrive_overlay';
    overlay.innerHTML = `
        <div class="spinner"></div>
        <div class="label">NFTDrive を開いています…</div>
    ...
    // nftdrive-ex.net/download.php リンクのクリックを検知
    // → 新タブにローディング画面を即表示してからリダイレクト（about:blank 対策）
    document.addEventListener('click', (e) => {
        const a = e.target.closest('a[href*="nftdrive-ex.net/download.php"]');
        if (!a) return;
        e.preventDefault();

        const url = a.href;

        // 新タブを開いてローディングHTMLを即書き込む
        const win = window.open('', '_blank');
        if (win) {
            win.document.write(`<!DOCTYPE html><html><head><meta charset="utf-8"><title>NFTDrive</title>
<style>
  body{margin:0;background:#111;display:flex;align-items:center;justify-content:center;
       height:100vh;flex-direction:column;gap:20px;font-family:sans-serif;color:#fff;}
  .s{width:52px;height:52px;border:5px solid rgba(255,255,255,0.2);
     border-top-color:#fff;border-radius:50%;animation:sp 0.75s linear infinite;}
  @keyframes sp{to{transform:rotate(360deg);}}
</style></head><body>
<div class="s"></div>
<div>NFTDrive を読み込んでいます…</div>
<script>window.location.href=${JSON.stringify(url)};</script>
</body></html>`);
            win.document.close();
        } else {
            // ポップアップブロック時はフォールバックで通常遷移
